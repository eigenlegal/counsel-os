...
# with a throwaway home, and screenshot Home / a thread / the reader / Settings /
# a vault search in light and dark. Usage: theme-shots.sh <label> <port>
set -e
LABEL="$1"; PORT="$2"
...
shot() {
  ...
  $B js "location.hash='#/settings'" >/dev/null; sleep 1.5; $B screenshot "$OUT/$LABEL-settings-$1.png" >/dev/null
  $B js "location.hash='#/vault'" >/dev/null; sleep 1.2
  $B fill 'input[placeholder*="Search"]' "audit rights" >/dev/null 2>&1
  $B js "const i=document.querySelector('input[placeholder*=Search]'); i.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}))" >/dev/null; sleep 1.8
  $B screenshot "$OUT/$LABEL-search-$1.png" >/dev/null
}
