# Agent plugin and legacy runtime guide

This guide covers the published Counsel OS agent plugin and its Markdown-vault workflows, plus the older standalone runtime. It does not describe the new SQLite workspace or native desktop app. For those, start with the [project README](../README.md) and [desktop build and release guide](desktop-release.md).
...
- **Claude Code, marketplace** (recommended): versioned, with updates through the plugin's update flow; no build toolchain.
...
This is the older runtime, not the new SQLite workspace or desktop app. For current desktop development and qualification, use the [desktop guide](desktop-release.md).
...
- **python-docx**, only for `clean_format` until it moves into the runtime. Reading `.docx`, extracting counterparty markup, checking a draft, writing redlines, comparing documents and comparing negotiation rounds need nothing beyond the runtime.
...
|--------|-------------|------------------------|
| `bun runtime/src/cli.ts docx apply [--track] [--author <name>] <original.docx> <redlines.json> [--out <output.docx>]` | Applies text replacements and margin comments to a `.docx` in TypeScript. With `--track`, edits are written as native Word tracked changes (`w:ins`/`w:del`, attributed and timestamped) — the output is the redline, no Word needed. Sees text inside hyperlinks and tracked insertions; refuses ambiguous matches until disambiguated; writes `<original>-redline-<date>.docx` beside the source and never overwrites. | Re-applying an edited redline spec without redoing the analysis; debugging why a specific replacement didn't land (it reports found/skipped per item). |
| `bun runtime/src/cli.ts docx compare <original.docx> <revised.docx> [--out <output.docx>] [--author <name>]` | Compares two independently edited documents and writes a native tracked-changes `.docx` attributed to `<author>` beside the original, in TypeScript on every platform — no Word, no AppleScript. Paragraphs align by similarity; table rows compare in place. | Producing a redline between two documents outside a counsel session; the counterparty returned a clean draft with no edit list. |
...
| `bun runtime/src/cli.ts docx rounds --ours <sent.docx> --theirs <returned.docx> [--base <docx>]` | Round-over-round negotiation comparison: classifies what happened to each of your counters as ACCEPTED, REVERTED, MODIFIED, or NEW changes they introduced. `--base` (the pre-redline baseline) sharpens the classification. | Round 2 and later of a negotiation; counsel runs it inside `read --redline` when a prior round exists. Run directly to see what survived. |

The redline flow counsel runs for you: analysis, then the `apply_redlines` tool with tracked changes on (native tracked changes and comments in one step); in the web UI the produced document appears in the conversation with a download.  `docx_compare` covers two documents that already exist independently. See `primitives/draft.md` and `primitives/redline-output.md`.
...
- The runtime (the web app) can also talk to other providers you add in Settings — OpenAI, Google Gemini, Mistral, Groq, xAI, OpenRouter, an Ollama or OpenAI-compatible server on your machine. Every provider row says where the text goes: `local · nothing leaves this machine`, or `cloud · text goes to <Company>` with a link to that vendor's terms. Paste a vendor's key on its row in Settings; it goes to your Keychain (a keyring or a 0600 file elsewhere), never into a config file or the vault. A key in the environment still works for headless use. A firm's own cloud account works too — Azure OpenAI, Amazon Bedrock, Google Vertex AI — with its credentials kept the same way, or found from the machine's AWS profile or gcloud login.
