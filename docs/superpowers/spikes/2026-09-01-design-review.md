...
| 4 | med | Vault tree › folder matter | The matter's documents were prettified (`Sample mutual nda` twice for the `.docx` and `.md`; `Counsel OS os redlines 2026 09 01`). | Two identical rows for two different files; every other file row in the tree shows the filename. | **Fixed.** Documents show their filenames, like the rest of the tree. |
