...
- **Anonymizer placeholders** read as names, not tags: `Party A`/`Party B` for the matter's client/counterparty, `Company N` / `Person N` for other entities, `person-n@example.com`, `555-01NN`, `$[amount N]`, `[date N]`. Authors on tracked changes become `Counsel OS`.
