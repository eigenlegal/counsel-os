...
| Run record surface | Per-step collapsible panel under the assistant turn (status, provider, primitives read, tools with ms, proposals, cost, duration); `GET /runs?thread=` on thread open; live from the stream while running | Spec-3 run record made visible |
...
| Provider `baseURL` bound | `https://…` only, except `http://` to a loopback host (`127.0.0.1`, `localhost`, `[::1]`) for local servers such as Ollama/vLLM; anything else is 400 | An authenticated PUT could otherwise point `apiKeyEnv` at an attacker host and exfiltrate an env var on the next test/step |
