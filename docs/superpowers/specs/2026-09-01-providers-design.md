...
**Addendum (step 5, built 2026-09-01) — the enterprise vendors.** `azure` (Azure OpenAI, `@ai-sdk/azure`), `bedrock` (Amazon Bedrock, `@ai-sdk/amazon-bedrock`) and `vertex` (Google Vertex AI, `@ai-sdk/google-vertex`; a `claude-…` model id routes to `@ai-sdk/google-vertex/anthropic`) join Layer A in a fourth group, `enterprise` (picker: *Hosted API · enterprise*), with `auth: 'azure' | 'sigv4' | 'gcp'`. Instead of `keyEnv` each record carries `fields: VendorField[]` — `{ name, label, secret, required, default?, env? }`. Non-secret fields (`resourceName`, `apiVersion`; `region`, `profile`; `project`, `location`) live on the registry entry as `extra`; secret fields (`apiKey`; `accessKeyId` + `secretAccessKey` + `sessionToken` or a Bedrock bearer `apiKey`; `serviceAccountJson` or an express `apiKey`) are ONE store item under `counsel-os/<id>` as `{ v: 1, fields }`. `enterprise.ts` resolves store → environment (the SDK's own variables; `AWS_PROFILE` or the entry's `profile` read from `~/.aws/credentials`; `GOOGLE_APPLICATION_CREDENTIALS` left for the SDK to open) → the SDK's default chain, and `keySet` gains `'default-chain'`. `PUT /providers/:id/key` takes `{ fields }` for these vendors, validated per vendor. Discovery: `azure` lists deployments (`/openai/deployments?api-version=2023-03-15-preview`, `api-key` header), `bedrock` signs `ListFoundationModels` with `aws4fetch` and falls back to a curated list, `vertex` is curated. The model part of the id is the deployment (Azure), the model id or inference profile (Bedrock), the model (Vertex). The UI mirrors the field sets and draws them under the row with the secret inputs masked and never re-rendered after save.
