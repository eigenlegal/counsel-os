...
export const PRACTICE_CAPABILITIES = [
  ...
  'Imported originals are retained in the Counsel OS workspace. Matter associations and saved outputs are workspace records, not synchronization with original folders. Local vault paths are context only: no external folder watcher, write-back, arbitrary filesystem access or shell/script execution is available.',
...
export function practiceCapabilityNotes(text: string): Array<{ title: string; detail: string }> {
  ...
  if (/embedded[ _-]?fonts?|font[ _-]?(?:embed|cleanup)|strip_font|font.*compress/i.test(text)) notes.push({ title: 'Font handling is built in',
    detail: 'Counsel OS compresses embedded fonts when saving Word files and preserves the fonts. It does not remove them or run an external cleanup script.' });
  if (/\b(?:vault|obsidian|legal_root|entities_path)\b|\/Users\/|file:\/\/|[A-Z]:\\/i.test(text)) notes.push({ title: 'External folders are not synchronized',
    detail: 'Imported files are retained in Counsel OS. Saving a folder path here does not connect that folder, file outputs there, or keep it in sync.' });
  if (/\b(?:execute|run|invoke)\b.{0,100}\b(?:script|python|shell)\b|\b\w+\.(?:py|sh)\b/i.test(text)) notes.push({ title: 'Local scripts are not executed',
    detail: 'Counsel OS can remember an external workflow, but cannot run its scripts. That instruction is not an enabled automation.' });
  return notes;
}
