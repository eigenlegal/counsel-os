...
export async function organizeImport(store: WorkspaceStore, provider: ModelProvider, batchId: string,
  ...
  try {
    for await (const event of provider.run({ tenant: 'workspace', tools: [], maxToolCalls: 1,
      ...
      system: `You are Counsel OS's import organization helper. Return only JSON with suggestions, exactly one per selected file.
...
Suggest destination, collection, matterId, matterTitle, whenToUse, reason, confidence and evidenceRef. Select the ID of a supporting excerpt from that same file's evidence list. Counsel OS retrieves the exact text; do not recreate or paraphrase an evidenceQuote. An excerpt reference proves only where text came from, not that a filing choice is correct. Preserve current choices unless the evidence or user's instruction supports a change. Use existing matter IDs only from the supplied candidates. Candidate titles are not proof of a match; distinguish the counterparty, project and document. Missing candidates do not mean no matching matter exists. Keep uncertain files unfiled, with low confidence and a useful reason. Never infer matter status, deadlines, signing authority or approval.
...
Context:\n${JSON.stringify({ files: files.map(file => ({ entryId: file.entryId, path: file.path, partial: file.partial,
  choice: { ...file.choice, profile: undefined }, evidence: evidence.get(file.entryId) })), candidateMatters: sharedMatters, ...(background ? { proposedGroups: background.groups } : {}) })}`,
      ...
      try {
        ...
        if (!background) throw new WorkspaceConflictError('Counsel OS returned an incomplete organization response. Your import choices are unchanged.');
      }
      if (!background && (items.length !== files.length || items.some(item => !files.some(file => file.entryId === (item as { entryId?: string })?.entryId))))
        throw new WorkspaceConflictError('Counsel OS did not return one suggestion per selected file. Your import choices are unchanged.');
