...
export async function draftPractice(store: WorkspaceStore, provider: ModelProvider,
  ...
  try {
    ...
    for await (const event of provider.run({ tenant: 'workspace', tools: [], maxToolCalls: 1, maxTokens: 6500,
      ...
      system: `You are Counsel's in-place drafting helper. Return only a JSON object with title, body, kind, question.
...
Context:\n${JSON.stringify(context)}`,
      ...
      if (event.type === 'done') {
        ...
        if (!result.success) throw new WorkspaceConflictError('Counsel could not produce a usable form draft. Your text is unchanged; try a more specific instruction.');
        if (input.target === 'instructions' && result.data.body.length > 4000)
          throw new WorkspaceConflictError('The proposed instructions are too long. Ask Counsel for a shorter version. Your text is unchanged.');
        return result.data;
      }
    }
