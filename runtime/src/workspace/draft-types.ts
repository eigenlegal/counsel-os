...
import { PracticeDocumentDraft } from './practice-document';

const Id = z.string().uuid();
export const DraftKey = z.union([z.enum(['working-preferences', 'practice-document']), z.string().regex(/^chat:[a-zA-Z0-9:-]{1,150}$/)]);
...
export const DraftWrite = z.object({ key: DraftKey, expectedRevisionId: Id.nullable(), writeId: Id,
  value: z.union([ChatDraft, PreferenceDraft, PracticeDocumentDraft]).nullable(),
}).strict().refine(v => v.value === null || (v.key === 'working-preferences'
  ? PreferenceDraft.safeParse(v.value).success : v.key === 'practice-document' ? PracticeDocumentDraft.safeParse(v.value).success : ChatDraft.safeParse(v.value).success), 'Draft type does not match its location.');
export interface SavedDraft { key: string; revisionId: string | null; writeId: string | null; value: ChatDraft | PreferenceDraft | PracticeDocumentDraft | null; updatedAt: string | null }
