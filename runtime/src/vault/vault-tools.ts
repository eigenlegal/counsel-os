...
export function vaultTools(store: VaultStore): ToolDef[] {
  ...
  const search: ToolDef<{ query: string }, unknown> = {
    name: 'vault_search',
    description: 'Search the vault by keyword. Files matching ALL the query words are preferred; if none match all of them, it falls back to files matching ANY word, ranked so a rarer word counts for more than a repeated common one. Matching is case-insensitive on literal substrings — no wildcards or boolean operators — and punctuation is not part of a word. File CONTENTS are searched for text files (.md, .txt, .json, .yaml, .yml, .csv) only; every other file (PDF, DOCX, images) matches on its filename or path, as do text files. Very common words (the, is, our, what, …) are ignored, unless the query is nothing but common words. Returns at most 50 hits, best first, each a path relative to the vault root with a snippet — a path-only match shows the path as its snippet. Fewer than 50 results means the search was not truncated; exactly 50 may mean it was, so narrow the query. An empty result means nothing matched any of those words — not that the topic is absent from the vault; try different words, or vault_list, before concluding a document does not exist.',
