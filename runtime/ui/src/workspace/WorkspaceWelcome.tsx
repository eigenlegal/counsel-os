...
export function WorkspaceWelcome({ data, changed, editProfile }: { data: Snapshot; changed: () => void; editProfile: () => void }) {
  ...
  return <section className="workspace-welcome" aria-label="Set up your workspace">
    ...
    <div className="welcome-step"><div><h2>Make it yours <small>Optional</small></h2>
      <p>{data.practiceDocument?.body ? 'Your saved practice context is ready to develop further.' : 'Tell Counsel what you do and how you like to work, or start with one preference. No required categories or questionnaire.'}</p>
      <p>Counsel proposes what to remember, including the name on your Word comments. You review it together before it applies to future work.</p></div>
      ...
      }}>Tell Counsel about your practice</button><button className="text-button" onClick={editProfile}>Write or paste text</button><button className="text-button" onClick={() => setSources(true)}>Use saved instructions</button></div>
    ...
    <div className="welcome-step"><div><h2>Bring in your files <small>Optional</small></h2>
      <p>Drop files or folders in any organization. Counsel can suggest where they belong. After importing, bring any existing instructions into your practice chat.</p>
      <p className="fine-print">Counsel keeps its own copies. Your original folders stay untouched and are not kept in sync.</p></div>
