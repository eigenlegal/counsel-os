...
export function PracticeDraftAssist({ value, matterId = null, apply, busyChanged, disabled = false, target = 'practice', kindLocked = false }: {
  ...
  return <FormDraftAssist value={value} apply={apply} busyChanged={busyChanged} disabled={disabled}
    placeholder={target === 'instructions' ? 'e.g. Explain that I prefer surgical NDA edits, preserving the other side’s wording where possible…' : 'Describe what to capture, paste an example, or ask Counsel OS to refine your wording…'}
