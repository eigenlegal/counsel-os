...
export function DesktopUpdates() {
  ...
  async function run(download: boolean) {
    if (download && !window.confirm('Download the verified installer? First save your work and a workspace backup. After downloading, quit Counsel before replacing the app. Nothing installs automatically.')) return;
    ...
    try {
      if (download && update) {
        ...
        const a = document.createElement('a'); a.href = result.downloadUrl; a.download = 'Counsel-update.dmg'; a.click(); setSaved(true);
  ...
  return <section className="settings-section"><h2>App updates</h2><p>{status ? `Counsel ${status.version} · build ${status.build}` : 'Checking this app’s version…'}</p>
    ...
    {update && <div><h3>Counsel {update.manifest.version}</h3><p>{update.manifest.notes}</p><p>Requires macOS {update.manifest.minMacOS} or later on Apple silicon.</p><button className="button" disabled={busy} onClick={() => void run(true)}>Download verified installer</button></div>}
    {busy && <button className="button" onClick={() => { controller.current?.abort(); setBusy(false); }}>Cancel</button>}
    {saved && <p role="status">Installer verified. Finish saving it, back up your workspace, then quit Counsel before replacing the app. No installation has been performed.</p>}
    <p>Before changing a workspace’s database format, Counsel creates and verifies a recovery backup. If that fails, the upgrade stops.</p>
