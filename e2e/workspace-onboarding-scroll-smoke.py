...
with sync_playwright() as p:
    ...
    expect(sidebar.get_by_role('link', name='Counsel chats', exact=True)).to_have_attribute('href', '#/home?view=history')
    ...
    expect(field).to_be_in_viewport()
    expect(welcome).to_have_count(0)
    sidebar.get_by_role('link', name='Counsel chats', exact=True).click()
    expect(page.get_by_role('region', name='Conversation history', exact=True)).to_be_visible()
