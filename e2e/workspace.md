...
`workspace-alignment-smoke.py` uses disposable default and `--empty` fixtures on ports 7458 and 7459. It checks 14 screens at 1920/1440/1024/390px: shared breadcrumb/banner/page gutters, left-anchored narrow pages, reader widths and nearby details, ordinary Chats-list scrolling layout, centered chat/setup, editor checkbox alignment, and actual onboarding wheel scrolling. It fails on horizontal overflow, page errors, external requests or model calls. Use `--browser webkit` for the second engine and `--artifacts PATH` to retain screenshots and measured geometry.
...
python /path/to/webapp-testing/scripts/with_server.py \
  ...
  -- python e2e/workspace-alignment-smoke.py
```

`workspace-practice-document-smoke.py` uses a disposable `--empty --practice-document` fixture on port 7465. It checks direct chat-first setup, the single free-form document, durable edit recovery, handoffs that do not auto-send, a scripted text/identity/Word-author proposal, explicit review, content-discovered import instructions, exact selected attachments, setup dismissal, and scrolling at 1440/390/320px. No private workspace or live model is used. `practice-document.test.ts` also covers legacy preservation, disabled sharing, full context in chat/drafting, exact long-document patches, atomic confirmation, stale edits/undo, offline name confirmation and backup/restore. Existing per-field preference smoke scripts describe the compatibility path before a unified document is saved.

`workspace-images-smoke.py` uses the disposable `--chat` fixture on port 7461. It draws a synthetic screenshot and tests drag/drop, clipboard paste, PNG/JPEG/WebP picker intake, authenticated thumbnails, draft reload, image-input transport on first/follow-up messages, visible context receipts and 1440/390/320px layout. `images.test.ts` checks original retention, type/size/dimension refusals, explicit-only context, Trash exclusion, adapter payloads, byte budgets, preview authorization and backup/restore. `workspace-images-check.ts --live --allow-plan-usage --provider codex|claude-code --model MODEL` reuses only that synthetic screenshot to check actual visual interpretation and follow-up in a disposable workspace; it uses the selected account's plan and never reads personal workspace files. No OCR, broad image retrieval, or support for every model is implied.

`bun e2e/workspace-practice-document-check.ts --live --allow-plan-usage --provider codex --model MODEL` opts into three live chat requests against an isolated synthetic workspace. It imports two ordinary instruction files, checks a real model's reviewed text/identity/Word-author proposal, confirms it locally, verifies the exact decoded context and behavior in a fresh chat without attachments, and exercises a narrow update plus undo. Original files must remain unchanged. The report and disposable database are retained in the printed temporary directory; no personal database is accepted. Codex `gpt-5.6-sol` passed all 15 checks on 2026-09-14. This qualifies the tested synthetic flow, not every model or arbitrary profile. The browser test also covers setup-modal dismissal during handoff; UI tests retain the modal on failed handoff and preserve unsaved text.

`workspace-web-smoke.py` runs on a disposable `--chat --web-fixture` server at port 7461 with a deterministic public-network dependency and scripted model. It follows a terms page's definitions link through real app tools, reads/cites both immutable sources, opens the exact quoted passage and saved original, and checks receipts/reload at 1440/390/320px. It makes no real website or model requests. `public-web.test.ts` covers private-address and redirect/DNS-rebinding rejection, URL/schema bounds, cancellation and inert extraction. `web-sources.test.ts` covers preserved original bytes, versions, backup/restore, scope, exact-read citation requirements, disclosure limits and per-turn limits. Live public-page transport is checked separately without private documents or model calls; a fetched page is not legal verification.

`workspace-composer-draft-smoke.py` runs against the isolated `--chat` fixture on port 7461. It holds actual autosave requests and verifies unchanged composer/textarea/control/footer geometry during debounce, saving, completion and repeated edits at 1440/390/320px, with and without an attachment. It also checks visible save errors, retry, and recovery after reload. No model sends or real workspace records are used. Run through `with_server.py` with `WORKSPACE_TEST_DIST` pointing to the tested UI build.
...
`workspace-recall-smoke.py` uses a fresh `--chat --recall` fixture on 7461. It asks about recruiting employees, checks that the saved nonsolicitation instruction is actually read/cited, and verifies **Response context → How Counsel OS searched**, reload, no unexpected network requests and 1440/390/320px layouts. No live model or user data is used. `recall-plan.test.ts` covers malformed/failed/timed-out planning, cancellation, bounded inputs, eight vocabulary gaps, scope-before-limit, historical/trashed/replaced records, Unicode passage windows, retry and backup/reopen.
...
`workspace-background-import-smoke.py` runs against a fresh `--chat` fixture on 7461. Thirteen synthetic files exercise automatic staged filing, two fresh batches, one successful isolated repair, one exhausted bad-evidence retry, grouped destinations, exceptions-first review, a single leave-exceptions-unfiled action, navigation/reload, read-only editing while running, separate final import and 1440/390/320px layouts. The file table is optional and the interface/engine versions must match. No real workspace or model is used. Screenshots are saved under `e2e/.tmp/workspace/background-import-*.png`.
...
`bun e2e/workspace-background-import-check.ts --live --allow-plan-usage --provider codex --model MODEL` uses a new retained synthetic workspace and the actual background worker. It checks eleven files across two initial requests, with at most one extra repair per file: consistent new NDA grouping, a distinct existing employment matter, unfiled company background, no deal-concession promotion, automatic clear-choice preparation and no import before confirmation. A previous pre-recovery Codex/Sol run passed the earlier eight checks; the revised live protocol has not been run for this change. It consumes the selected plan and must be explicitly enabled. `import-organization-jobs.test.ts` covers 121-file preparation, protected corrections, bounded partial recovery, provider errors, pause/reopen, locality/profile exclusion, verified backups and migration/resume of a synthetic schema-19 import with 48 retained suggestions. The selected-only organization smoke remains a regression check.
...
`bun e2e/workspace-codex-preflight.ts` separately exercises the real installed CLI against loopback fake model/MCP endpoints with no login credentials. It checks direct Counsel OS tool availability and that the code-mode host is denied. Passing this bounded test is not a general sandbox guarantee.
...
September 14 extension: the native corpus also includes tracked replacements crossing hyperlinks and exact displayed bullet labels. Preserve/accept/reject/clean round trips, all seven standalone OOXML validations, native tracking validation and all four rendered pages passed. Word may serialize retained hyperlink targets as HYPERLINK fields; qualification checks both representations. `bun test runtime/src/docx/hyperlink-redline.test.ts runtime/src/docx/package.test.ts runtime/src/workspace/redlines.test.ts` additionally checks same/cross-link edits, unchanged target preservation, plain-text replacements, earlier-revision refusal, clean proposals, exact list-label matching, ambiguity refusal and a compressed embedded-font fixture exceeding 5 MB when expanded. No private document or model call is needed for this corpus.
