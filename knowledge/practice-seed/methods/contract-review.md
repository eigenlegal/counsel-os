...
Reference guide for reviewing commercial contracts (SaaS, services, licensing, procurement). Use as a coverage checklist when the `evaluate` primitive runs a full document review.

## Clause types to evaluate

For each clause type, load the position file for your standard and the library file for counter-language:

| Clause Category | Position File | Library Reference |
|---|---|---|
...
|---|---|---|
| Low | <$100K ACV, standard terms | Spot-check the 5 highest-impact clause types: limitation of liability, data protection, indemnification, termination, IP ownership |
| Medium | $100K-$1M ACV or non-standard terms | All 13 clause types. Full compliance check. |
| High | >$1M ACV, strategic relationship, significant data processing | All 13 clause types. Deep compliance check against all triggered law/ areas. Escalation threshold review. |

## For redlined documents

When reviewing a document that already has tracked changes:
- Assess what the redlines fixed vs. what remains unaddressed
- Don't re-evaluate clauses that are acceptable post-redline
- Check if tracked changes introduced new internal inconsistencies
- Note who made the changes and whether the rationale is clear
