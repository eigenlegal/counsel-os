...
**We won't accept:** No escrow provision for mission-critical software, source code held by vendor "in trust" (not independent agent), release conditions requiring vendor consent, no update obligation, or restrictive license (read-only, no modification) upon release.
**Auto-escalate:** No escrow for mission-critical vendor-retained software; release requires vendor consent; no verification testing; deposit not updated with releases; license upon release too restrictive for maintenance; escrow excludes build tools and documentation; vendor can terminate escrow unilaterally.
