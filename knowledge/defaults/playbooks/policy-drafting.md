# Policy Drafting Playbook

## When to Use
Use this playbook when creating or substantially revising an internal company policy. This includes compliance policies, operational policies, HR policies, information security policies, and any formalized standard operating procedures that require legal review and approval.

## Prerequisites
- Clear understanding of the policy objective and the problem it addresses
- Identification of the policy sponsor (executive owner)
- Relevant legal and regulatory requirements identified from knowledge/law/areas/
- Existing related policies reviewed for consistency and overlap
- Input gathered from key stakeholders (HR, IT, Compliance, Operations, etc.)
- Benchmark examples from peer companies or industry standards (if available)

## Process

### Step 1: Define the Policy Objective
Before drafting, clearly establish:
- **Problem Statement:** What gap, risk, or regulatory requirement does this policy address?
- **Scope:** Who does this policy apply to (employees, contractors, subsidiaries, affiliates, third parties)?
- **Jurisdiction:** Where does this policy apply (global, US-only, specific regions)?
- **Authority:** What legal or regulatory authority mandates this policy (if any)?
- **Related Policies:** What existing policies intersect, and how does this policy fit into the policy hierarchy?

### Step 2: Research Requirements
Gather all requirements that must be reflected in the policy:
- Statutory and regulatory requirements
- Industry standards and best practices (ISO, NIST, SOC, etc.)
- Contractual obligations to customers or partners
- Board directives or management mandates
- Insurance requirements
- Prior audit findings or remediation commitments

### Step 3: Outline the Policy Structure
Use a consistent policy template:
1. **Title and Version**
2. **Effective Date**
3. **Policy Owner** (executive sponsor)
4. **Purpose** (why the policy exists)
5. **Scope** (who and what it covers)
6. **Definitions** (key terms)
7. **Policy Statement** (the core requirements)
8. **Roles and Responsibilities** (who does what)
9. **Procedures** (how to comply)
10. **Exceptions** (how to request an exception)
11. **Enforcement and Consequences** (what happens if violated)
12. **Related Policies and References**
13. **Revision History**

### Step 4: Draft the Policy
Drafting best practices:
- Use clear, direct language accessible to the target audience
- Avoid legalese — policies should be understandable by non-lawyers
- Use "must" for mandatory requirements, "should" for best practices, "may" for optional actions
- Be specific about obligations — vague policies are unenforceable and unhelpful
- Include practical examples where they aid understanding
- Define all technical or legal terms in the definitions section
- Ensure consistency with existing policies and the employee handbook
- Consider translation requirements for global policies

### Step 5: Stakeholder Review
Circulate the draft for input:
- Legal review for regulatory compliance and legal accuracy
- HR review for employment law implications and consistency with the handbook
- IT/Security review for technical feasibility (if applicable)
- Business unit review for operational practicality
- Compliance review for alignment with the compliance program
- Privacy review if the policy involves personal data processing
- Incorporate feedback and resolve conflicting input

### Step 6: Approval and Adoption
Formalize the policy:
- Obtain executive sponsor sign-off
- Obtain legal sign-off
- If required, obtain board or committee approval
- Assign a version number and effective date
- Publish in the policy repository / intranet
- Communicate to all affected individuals
- Conduct training sessions if the policy introduces new obligations
- Obtain acknowledgment of receipt from affected personnel

### Step 7: Maintenance
Establish a lifecycle management plan:
- Schedule periodic reviews (at least annually)
- Monitor for regulatory changes that require policy updates
- Track exception requests and policy violation trends
- Update the policy when triggered by regulatory changes, audit findings, or business changes
- Maintain a complete revision history

## Output Format
1. **Policy Document:** Formatted per the template in Step 3
2. **Regulatory Mapping:** Matrix showing which requirements the policy addresses
3. **Communication Plan:** How the policy will be rolled out and to whom
4. **Training Materials:** If required, key points and FAQs for training
5. **Acknowledgment Form:** Template for personnel to acknowledge receipt

## Calibration
- **Simple:** Minor policy update or low-complexity operational policy. Target: 1-2 weeks from draft to adoption.
- **Standard:** New policy in a moderately regulated area (information security, acceptable use, travel). Target: 3-6 weeks.
- **Complex:** Highly regulated area (anti-corruption, whistleblower, data privacy, insider trading), global scope, board approval required. Target: 2-3 months. Consider engaging outside counsel for regulatory review.
