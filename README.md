...
|--------|-------------|------------------------|
| `scripts/apply_redlines.py [--track] <original.docx> <redlines.json> <output.docx>` | Applies text replacements and margin comments to a `.docx` (python-docx). With `--track`, edits are written as native Word tracked changes (`w:ins`/`w:del`, attributed and timestamped) — the output is the redline, no Word needed. Sees text inside hyperlinks and tracked insertions; refuses ambiguous matches until disambiguated. | Re-applying an edited redline spec without redoing the analysis; debugging why a specific replacement didn't land (it reports found/skipped per item). |
...
The redline flow counsel runs for you: analysis, then `apply_redlines.py --track` (native tracked changes and comments in one step). `word_compare.sh` remains the alternative engine for comparing two documents that already exist independently. See `primitives/draft.md` and `primitives/redline-output.md`.
