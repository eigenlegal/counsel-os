...
- The runtime (the web app) can also talk to other providers you add in Settings — OpenAI, Google Gemini, Mistral, Groq, xAI, OpenRouter, an Ollama or OpenAI-compatible server on your machine. Every provider row says where the text goes: `local · nothing leaves this machine`, or `cloud · text goes to <Company>` with a link to that vendor's terms. Keys are read from the environment variable named in `providers.yaml` (or the vendor's usual one) until the app stores them itself.
