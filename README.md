...
Native Word redlines are written by the runtime itself (TypeScript, every platform): tracked changes with author, date and comments, no Python and no Microsoft Word. Comparing two independently edited documents still uses Word's Compare on macOS until the runtime's own compare lands. Memos, summaries, and emails come back as formatted markdown everywhere.
...
- **python-docx**, only for `diff_rounds` (round-over-round comparison) and `clean_format` until those move into the runtime. Reading `.docx`, extracting counterparty markup, checking a draft and writing redlines need nothing beyond the runtime.
...
The redline flow counsel runs for you: analysis, then the `apply_redlines` tool with tracked changes on (native tracked changes and comments in one step); in the web UI the produced document appears in the conversation with a download. `word_compare.sh` remains the alternative engine for comparing two documents that already exist independently. See `primitives/draft.md` and `primitives/redline-output.md`.
