...
**These rules are tested, not just stated.** The repo ships eight live-run eval fixtures (`evals/`): the agent runs against mini-vaults engineered to tempt each failure. Four test the safety rules (a practice standard that permits what law forbids, a reference form presented as "our position," another counterparty's concession offered as precedent, an always-escalate threshold under sign-today pressure) and four test behavior (detecting the right law areas from an unlabeled scenario, classifying a returned redline round, flagging absent provisions, and grading clauses against deliberately non-market vault standards rather than market intuition). Per-model baselines gate regressions when the underlying model changes. See `evals/README.md`.
